...
def train_and_predict_lr(
    ...
    """Train Linear Regression on daily samples and predict daily targets in a window."""
    ...
    # (Removed previous-day merge; we will use next-day supply for borrow prediction)

    # Also fetch supply_rate(t+1) for CSV output column `supply_rate`
    predict_df['date_next'] = predict_df['date_day'] - pd.Timedelta(days=1)
    sup_next_series = daily[['date_day', 'supply_rate']].rename(
        columns={'date_day': 'date_next', 'supply_rate': 'supply_rate_next'}
    )
    predict_df = predict_df.merge(sup_next_series, on='date_next', how='left')

    # Compute borrow(t) = supply_rate(t+1) * utilization_pred(t)
    predict_df['borrow_rate_prediction'] = predict_df['supply_rate_next'] * predict_df['utilization']

    # Final output
    out_df = (
        predict_df[
            ['date_day', 'supply_rate_next', 'utilization', 'borrow_rate_prediction']
        ]
        .dropna(subset=['supply_rate_next'])
        .rename(
            columns={
                'date_day': 'timestamp',
                'supply_rate_next': 'supply_rate',
            }
        )
...
def main():
    print('🔥 Linear Regression daily predictor (168×6 lags, per-day z-score) — borrow(t)=supply(t+1)*util_pred(t)')
    ...
    for filename, proto in protocols.items():
        ...
        preds = train_and_predict_lr(csv_path)
        if len(preds) == 0:
            continue
